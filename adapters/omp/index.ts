...
const RUNTIME = "omp";
const HOME = homedir();
const REGISTRY_PATH = `${HOME}/.omp/agent/opencode-import-registry.json`;
...
async function importOne(ctx: ExtensionCommandContext, parsed: ParsedArgs, sessionId: string): Promise<{ imported: boolean; session?: OpenCodeSessionRow; message?: string }> {
	...
	const sessionDir = OmpSessionManager.getDefaultSessionDir(targetCwd);
...
async function selectAndImport(ctx: ExtensionCommandContext, parsed: ParsedArgs, search?: string): Promise<void> {
	if (!ctx.hasUI) {
...
function handleList(ctx: ExtensionCommandContext, parsed: ParsedArgs, search?: string): void {
	const registry = loadImportRegistry(parsed.registryPath);
	const sessions = listOpenCodeSessions(parsed.dbPath, { search, cwd: parsed.cwd, since: parsed.since, updatedSince: parsed.updatedSince, limit: parsed.limit, mainOnly: !parsed.includeDelegated });
...
async function handleOpen(ctx: ExtensionCommandContext, parsed: ParsedArgs, sessionId: string | undefined): Promise<void> {
	...
	if (!entry) {
		ctx.ui.notify(`No imported OMP session for ${sessionId}`, "warning");
		return;
	}
	const result = await ctx.switchSession(entry.targetSessionFile);
	ctx.ui.notify(result.cancelled ? "Switch session cancelled" : `Opened imported OpenCode session ${sessionId}`, "info");
}

export default function opencodeImportExtension(pi: ExtensionAPI): void {
	pi.registerCommand("opencode-import", {
		description: "Import OpenCode sessions into native OMP sessions",
